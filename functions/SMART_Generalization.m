...
function [results,events] = RunGeneralizationBlock(cfg,state,textures,audio,generalizationTrials,mode)
    ...
    textures.empty = SMART_Initialize('CreateBoxesTexture',state.window,cfg, ...
        windowWidth,windowHeight,textures.layout,0,false,true,0,cfg.generalizationTrials);
    textures.generalizationEmpty = SMART_Initialize('CreateBoxesTexture', ...
        state.window,cfg,windowWidth,windowHeight,textures.layout,-1,false,true,0,cfg.generalizationTrials);
    ...
    for repetition = 1:cfg.generalizationRepetitions
        ...
        for index = 1:numel(order)
            ...
            [trialResult,trialEvents] = SMART_Generalization('RunGeneralizationTrial', ...
                cfg,state,textures,audio,trial,mode,repetition,index,globalTrial);
            ...
            textures.empty = SMART_Initialize('CreateBoxesTexture',state.window, ...
                cfg,windowWidth,windowHeight,textures.layout,0,false,true,globalTrial,cfg.generalizationTrials);
            textures.generalizationEmpty = SMART_Initialize('CreateBoxesTexture', ...
                state.window,cfg,windowWidth,windowHeight,textures.layout,-1,false,true,globalTrial,cfg.generalizationTrials);
...
function [result,events] = RunGeneralizationTrial(cfg,state,textures,audio, ...
    trial,mode,repetition,trialInPass,globalTrial)
    ...
    [soundOnset,triggerEvents] = SMART_Display('PresentTriggeredTexture',cfg, ...
        state,textures.empty,cfg.trigger.generalization,phase,blockNumber,globalTrial,'trial_onset',startTime,mode);
    ...
    [~,pressEvents] = SMART_Display('PresentTriggeredTexture',cfg,state, ...
        textures.empty,responseTrigger,phase,blockNumber,globalTrial,'response_press',pressTime,mode);
    ...
    result = SMART_Participant('MakeTrialResult',cfg,trial,phase,repetition, ...
        globalTrial,expectedResponse,pressedResponse,correct,reactionTime, ...
        slow,earlyPressDetected,soundOnset,soundOffset,responseOnset,pressTime);
end
